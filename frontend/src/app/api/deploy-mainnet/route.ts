...
export async function POST(request: NextRequest) {
    try {
        ...
        const { network, deployMode, selectedFeature, selectedContract, privateKey } = body;
        ...
        // Determine which script to run based on deploy mode
        ...
        console.log(`Executing mainnet script: ${scriptPath} with env: ${JSON.stringify({ ...env, PRIVATE_KEY: '***HIDDEN***' })}`);
        const result = await executeDeploymentScript(scriptPath, env, network);
        ...
        return NextResponse.json(
            { error: 'Lỗi trong quá trình deploy lên mainnet' },
            { status: 500 }
        );
    }
}

async function executeDeploymentScript(scriptPath: string, env: NodeJS.ProcessEnv, network: string): Promise<any> {
    return new Promise((resolve, reject) => {
        // For mainnet, use hardhat run with network parameter
        const child = spawn('npx', ['hardhat', 'run', scriptPath, '--network', network], {
        ...
        child.on('close', (code) => {
            console.log(`Mainnet script execution completed with code: ${code}`);
            ...
            if (code === 0) {
                resolve({
                    success: true,
                    output: stdout,
                    message: 'Mainnet deployment completed successfully'
                });
            } else {
                reject({
                    success: false,
                    error: stderr || 'Mainnet deployment failed',
                    code
                });
            }
        });
