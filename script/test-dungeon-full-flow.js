...
async function main() {
        ...
        if (!playerExists) {
            ...
            try {
                ...
                if (error.message.includes("already initialized") || error.message.includes("already exists")) {
                    console.log("   ⚠️  Player creation failed, verifying if player exists...");
                    try {
                        playerData = await playerLogic.getPlayerData(player1.address);
                        if (playerData.playerAddress === player1.address) {
        ...
        try {
            ...
            try {
                ...
                if (itemData.itemId > 0) {
                    itemExists = true;
                    console.log(`   ✅ Item ${item.name} (ID: ${item.id}) already exists`);
                }
            } catch (error) {
                // Item doesn't exist
            }
    ...
    async function addStagesToDungeon(dungeonId, stages) {
        console.log(`   • Adding ${stages.length} stages to dungeon ${dungeonId}...`);
        ...
        for (const stage of stages) {
            try {
                ...
                if (error.message.includes("already exists") || error.message.includes("Stage already exists")) {
                    console.log(`     ℹ️  Stage ${stage.stageNumber} already exists`);
                } else {
                    console.error(`     ❌ Failed to add stage ${stage.stageNumber}:`, error.message);
                    throw error;
                }
            }
        }
    ...
    try {
        ...
        try {
            ...
            for (const stage of dungeon.stages) {
                const multiplierDecimal = (Number(stage.rewardMultiplier) / 10000).toFixed(2);
                console.log(`       - Stage ${stage.stageNumber}: multiplier ${stage.rewardMultiplier} (${multiplierDecimal}x)`);
            }
        } catch (error) {
    ...
    try {
        const dungeonBalance = await ethers.provider.getBalance(dungeonLogicAddress);
        ...
        if (dungeonBalance < minBalance) {
            ...
            console.log(
                `   ✅ DungeonLogic already has sufficient balance: ${ethers.formatEther(dungeonBalance)} ETH`
            );
        }
    } catch (error) {
    ...
    try {
        ...
        console.log(`   • Has resource requirements: ${hasResources ? "YES" : "NO"}`);
    ...
    try {
        ...
        try {
            ...
            const dungeonBalanceBefore = await ethers.provider.getBalance(dungeonLogicAddress);
            ...
            console.log(`     • Player ETH Balance: ${ethers.formatEther(playerEthBefore)} ETH`);
            console.log(`     • DungeonLogic ETH Balance: ${ethers.formatEther(dungeonBalanceBefore)} ETH`);
        } catch (error) {
        ...
        try {
            ...
            for (const item of inventoryBefore) {
                console.log(`       - Item ${item.itemId}: quantity=${item.quantity}, durability=${item.durability}`);
            }
        ...
        console.log(`     - Equipment Quantities: ${equipmentQuantities.join(", ")}`);
        console.log(`     - Bet amount: ${ethers.formatEther(betAmount)} ETH`);

        const playerEthBeforeStart = await ethers.provider.getBalance(player1.address);
        ...
        const playerEthAfterStart = await ethers.provider.getBalance(player1.address);
        ...
        console.log(`     - Bet Amount: ${ethers.formatEther(session.betAmount)} ETH`);
        ...
        try {
            ...
            if (playerBefore) {
                ...
                console.log(`     • Mana: ${playerAfter.mana} (was: ${playerBefore.mana}, diff: ${Number(playerAfter.mana) - Number(playerBefore.mana)})`);
                console.log(`     • Max Mana: ${playerAfter.maxMana}`);
                console.log(`     • Sunlight: ${playerAfter.sunlight} (was: ${playerBefore.sunlight}, diff: ${Number(playerAfter.sunlight) - Number(playerBefore.sunlight)})`);
                console.log(`     • Sunny: ${playerAfter.sunny} (was: ${playerBefore.sunny}, diff: ${Number(playerAfter.sunny) - Number(playerBefore.sunny)})`);
                console.log(`     • Player ETH Balance: ${ethers.formatEther(playerEthAfterStart)} ETH (was: ${ethers.formatEther(playerEthBeforeStart)} ETH, diff: ${ethers.formatEther(playerEthBeforeStart - playerEthAfterStart)} ETH)`);
            } else {
                console.log(`     • Level: ${playerAfter.level}`);
                ...
                console.log(`     • Player ETH Balance: ${ethers.formatEther(playerEthAfterStart)} ETH`);
            }
        } catch (error) {
            console.log(`     ⚠️  Could not get player data: ${error.message}`);
        }
        ...
        try {
            ...
            for (const item of inventoryAfter) {
                if (inventoryBefore && inventoryBefore.length > 0) {
                    const beforeItem = inventoryBefore.find(i => i.itemId === item.itemId);
                    const beforeQty = beforeItem ? beforeItem.quantity : 0;
                    console.log(`       - Item ${item.itemId}: quantity=${item.quantity} (was: ${beforeQty}, diff: ${Number(item.quantity) - Number(beforeQty)})`);
                } else {
                    console.log(`       - Item ${item.itemId}: quantity=${item.quantity}, durability=${item.durability}`);
                }
            }
    ...
    try {
        ...
        try {
            ...
            const dungeonBalanceBeforeEnd = await ethers.provider.getBalance(dungeonLogicAddress);
            ...
            console.log(`     • DungeonLogic ETH Balance: ${ethers.formatEther(dungeonBalanceBeforeEnd)} ETH`);
        } catch (error) {
            console.log(`     ⚠️  Could not get player data: ${error.message}`);
        }
        ...
        try {
            const dungeon = await dungeonLogic.getDungeon(testDungeonId);
            if (dungeon.stages && dungeon.stages.length > 0) {
                const stage = dungeon.stages.find(s => Number(s.stageNumber) === stageNumber);
                if (stage) {
        ...
        if (expectedRewardMultiplier > 0) {
            console.log(`     - Expected Reward Multiplier: ${expectedRewardMultiplier} (${(expectedRewardMultiplier / 10000).toFixed(2)}x)`);
        }
        ...
        console.log(`     - Stage Number: ${session.stageNumber}`);
        console.log(`     - Reward Multiplier: ${rewardMultiplier} (${rewardMultiplierDecimal.toFixed(2)}x)`);
        if (session.hasBet && session.betAmount > 0) {
            const betReward = (session.betAmount * BigInt(rewardMultiplier)) / BigInt(10000);
            console.log(`     - Bet Amount: ${ethers.formatEther(session.betAmount)} ETH`);
            console.log(`     - Expected Bet Reward: ${ethers.formatEther(betReward)} ETH (${rewardMultiplierDecimal.toFixed(2)}x)`);
        }
        ...
        for (let i = 0; i < session.rewardItemIds.length; i++) {
            console.log(`       - Item ${session.rewardItemIds[i]}: quantity=${session.rewardQuantities[i]}`);
        }
        ...
        try {
            ...
            if (playerBeforeEnd) {
                ...
                console.log(`     • Mana: ${playerAfterEnd.mana} (was: ${playerBeforeEnd.mana}, diff: ${Number(playerAfterEnd.mana) - Number(playerBeforeEnd.mana)})`);
                console.log(`     • Sunlight: ${playerAfterEnd.sunlight} (was: ${playerBeforeEnd.sunlight}, diff: ${Number(playerAfterEnd.sunlight) - Number(playerBeforeEnd.sunlight)})`);
                console.log(`     • Sunny: ${playerAfterEnd.sunny} (was: ${playerBeforeEnd.sunny}, diff: ${Number(playerAfterEnd.sunny) - Number(playerBeforeEnd.sunny)})`);
            } else {
                console.log(`     • Level: ${playerAfterEnd.level}`);
        ...
        console.log("\n   📦 INVENTORY AFTER END:");
        try {
            const inventoryAfterEnd = await inventoryLogic.getInventory(player1.address);
            ...
            for (const item of inventoryAfterEnd) {
                if (inventoryBeforeEnd && inventoryBeforeEnd.length > 0) {
                    const beforeItem = inventoryBeforeEnd.find(i => i.itemId === item.itemId);
                    const beforeQty = beforeItem ? beforeItem.quantity : 0;
                    console.log(`       - Item ${item.itemId}: quantity=${item.quantity} (was: ${beforeQty}, diff: ${Number(item.quantity) - Number(beforeQty)})`);
                } else {
                    console.log(`       - Item ${item.itemId}: quantity=${item.quantity}`);
    ...
    try {
        ...
        if (sessionBeforeClaim.hasBet && sessionBeforeClaim.isCompleted && rewardMultiplier > 0) {
            expectedBetReward = (sessionBeforeClaim.betAmount * BigInt(rewardMultiplier)) / BigInt(10000);
        }

        // Lấy ETH balance trước khi claim
        const playerEthBeforeClaim = await ethers.provider.getBalance(player1.address);
        const dungeonBalanceBeforeClaim = await ethers.provider.getBalance(dungeonLogicAddress);

        console.log("\n   💰 ETH Balance BEFORE CLAIM:");
        console.log(`     • Player ETH: ${ethers.formatEther(playerEthBeforeClaim)} ETH`);
        console.log(`     • DungeonLogic ETH: ${ethers.formatEther(dungeonBalanceBeforeClaim)} ETH`);

        if (expectedBetReward > 0) {
            console.log(`     • Expected Bet Reward: ${ethers.formatEther(expectedBetReward)} ETH (${rewardMultiplierDecimal.toFixed(2)}x)`);
        }
        ...
        const playerEthAfterClaim = await ethers.provider.getBalance(player1.address);
        const dungeonBalanceAfterClaim = await ethers.provider.getBalance(dungeonLogicAddress);
        ...
        console.log(`     • Player ETH: ${ethers.formatEther(playerEthAfterClaim)} ETH`);
        console.log(`     • DungeonLogic ETH: ${ethers.formatEther(dungeonBalanceAfterClaim)} ETH`);
        console.log(`\n   📊 ETH Balance Changes:`);
        console.log(`     • Player ETH Change: ${ethers.formatEther(playerEthDiff)} ETH ${playerEthDiff >= 0 ? "(+)" : "(-)"}`);
        console.log(`     • DungeonLogic ETH Change: ${ethers.formatEther(dungeonEthDiff)} ETH ${dungeonEthDiff >= 0 ? "(+)" : "(-)"}`);
        ...
        if (expectedBetReward > 0) {
            ...
            console.log(`     • Net Reward (after gas): ${ethers.formatEther(netReward)} ETH`);

            if (netReward >= expectedReward * BigInt(99) / BigInt(100)) { // Allow 1% tolerance
                console.log(`     ✅ Reward matches expected! (within tolerance)`);
            } else {
                console.log(`     ⚠️  Reward mismatch! Expected: ${ethers.formatEther(expectedReward)}, Got: ${ethers.formatEther(netReward)}`);
            }
        }
    ...
    // === TEST 5: TEST LOSE SCENARIO (Stage 0) ===
    console.log("\n5️⃣ TEST: Testing LOSE Scenario (Stage 0)...");
    ...
    try {
        ...
        // End session with stage 0 (lost)
        console.log("   • Ending session as LOSE (stage 0)...");
        ...
        console.log(`     - Reward Multiplier: ${rewardMultiplier2} (${rewardMultiplierDecimal2.toFixed(2)}x)`);
        console.log(
            `     - Note: Reward multiplier = 0 means no bet reward (lost)`
        );
    } catch (error) {
    ...
    const testStages = [
        ...
        { stageNumber: 3, rewardMultiplier: 3000, description: "Low-Medium (0.3x)" },
        { stageNumber: 7, rewardMultiplier: 15000, description: "High (1.5x)" },
        { stageNumber: 10, rewardMultiplier: 30000, description: "Very High (3.0x)" },
    ];
    ...
    for (let i = 0; i < testStages.length; i++) {
        ...
        try {
            console.log(`\n   📋 Test Case ${i + 1}: Stage ${testStage.stageNumber} (${testStage.description})`);

            // Start new session
            console.log(`   • Starting new dungeon session for stage ${testStage.stageNumber}...`);
            const startTx = await dungeonLogic
                .connect(player1)
                .startDungeon(testDungeonId, [1001], [1], {
                    value: ethers.parseEther("100"),
            ...
            console.log(`     - Expected Reward Multiplier: ${testStage.rewardMultiplier} (${expectedMultiplierDecimal.toFixed(2)}x)`);
            console.log(`     - Actual Reward Multiplier: ${actualRewardMultiplier} (${actualMultiplierDecimal.toFixed(2)}x)`);

            if (actualRewardMultiplier === testStage.rewardMultiplier) {
                console.log(`     ✅ Reward multiplier matches!`);
            } else {
                console.log(`     ⚠️  Reward multiplier mismatch! Expected: ${testStage.rewardMultiplier}, Got: ${actualRewardMultiplier}`);
            }

            // Calculate bet reward if applicable
            let expectedBetReward = BigInt(0);
            if (sessionData.hasBet && sessionData.isCompleted && actualRewardMultiplier > 0) {
                expectedBetReward = (sessionData.betAmount * BigInt(actualRewardMultiplier)) / BigInt(10000);
                console.log(`     - Bet Amount: ${ethers.formatEther(sessionData.betAmount)} ETH`);
                console.log(`     - Expected Bet Reward: ${ethers.formatEther(expectedBetReward)} ETH (${actualMultiplierDecimal.toFixed(2)}x)`);
            }
            ...
            if (sessionData.isCompleted && expectedBetReward > 0) {
                ...
                console.log(`     • Player ETH Before: ${ethers.formatEther(playerEthBeforeClaim)} ETH`);
                console.log(`     • DungeonLogic ETH Before: ${ethers.formatEther(dungeonBalanceBeforeClaim)} ETH`);

                // Claim rewards
                const claimTx = await dungeonLogic.connect(player1).claimRewards(newSessionId);
                const claimReceipt = await claimTx.wait();
                ...
                console.log(`     • Player ETH After: ${ethers.formatEther(playerEthAfterClaim)} ETH`);
                console.log(`     • DungeonLogic ETH After: ${ethers.formatEther(dungeonBalanceAfterClaim)} ETH`);
                console.log(`\n     📊 ETH Balance Changes:`);
                console.log(`       • Player ETH Change: ${ethers.formatEther(playerEthDiff)} ETH ${playerEthDiff >= 0 ? "(+)" : "(-)"}`);
                console.log(`       • DungeonLogic ETH Change: ${ethers.formatEther(dungeonEthDiff)} ETH ${dungeonEthDiff >= 0 ? "(+)" : "(-)"}`);
                console.log(`       • Gas Fee: ${ethers.formatEther(gasUsed)} ETH`);
                console.log(`       • Net Reward (after gas): ${ethers.formatEther(netReward)} ETH`);
                console.log(`       • Expected Reward: ${ethers.formatEther(expectedBetReward)} ETH`);

                if (netReward >= expectedBetReward * BigInt(99) / BigInt(100)) {
                    console.log(`       ✅ Reward matches expected! (within tolerance)`);
                } else {
                    console.log(`       ⚠️  Reward mismatch! Expected: ${ethers.formatEther(expectedBetReward)}, Got: ${ethers.formatEther(netReward)}`);
                }
            }
        } catch (error) {
            console.error(`   ❌ Failed to test stage ${testStage.stageNumber}:`, error.message);
    ...
    console.log(`   ✅ Dungeon setup: Complete (10 stages with different multipliers)`);
    console.log(`   ✅ Session 1 (WIN): Started, Ended, Claimed`);
    console.log(`   ✅ Session 2 (LOSE): Started, Ended`);
    console.log(`   ✅ Session 3-6 (Multiple Stages): Tested ${testStages.length} different stages`);
    console.log(`\n📋 Session IDs:`);
    ...
    if (sessionIds && sessionIds.length > 0) {
        ...
        sessionIds.forEach((id, index) => {
            console.log(`     - Stage ${testStages[index].stageNumber} (${testStages[index].description}): ${id}`);
        });
    }
    ...
    try {
        ...
        dungeon.stages.forEach((stage) => {
            const multiplierDecimal = (Number(stage.rewardMultiplier) / 10000).toFixed(2);
            console.log(`     - Stage ${stage.stageNumber}: ${stage.rewardMultiplier} (${multiplierDecimal}x)`);
        });
    } catch (error) {
