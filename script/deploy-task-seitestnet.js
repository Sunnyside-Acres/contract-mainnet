...
async function main() {
  console.log("🚀 Bắt đầu deploy Task System lên Sei Testnet...");
  ...
  // Xác nhận deploy
  console.log("\n⚠️  Bạn đang deploy Task System lên SEI TESTNET!");
  console.log("💰 Chi phí deploy sẽ thấp hơn mainnet, phù hợp để test");
  ...
  try {
    const testnetPath = "./deployed/contract-addresses-seitestnet.json";
    if (fs.existsSync(testnetPath)) {
      existingAddresses = JSON.parse(fs.readFileSync(testnetPath, "utf8"));
      console.log("✅ Đọc từ file seitestnet thành công");
    } else {
      throw new Error("Không tìm thấy file địa chỉ contract testnet");
    }
  } catch (error) {
    console.error("❌ Lỗi đọc file địa chỉ:", error.message);
    console.log(
      "💡 Hãy chạy script deploy testnet đầy đủ trước để có file địa chỉ"
    );
    process.exit(1);
  }
  ...
  // Lưu vào file
  const deploymentPath = `./deployed/contract-addresses-seitestnet.json`;
  ...
  console.log("\n🎉 Deploy Task System lên Sei Testnet hoàn tất thành công!");
  ...
  console.log("💰 Chi phí deploy đã được tối ưu cho testnet");
  console.log("🔒 Contracts đã được verify và sẵn sàng test");
