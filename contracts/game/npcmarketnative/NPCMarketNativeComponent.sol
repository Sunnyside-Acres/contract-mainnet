...
/**
 * @title NPCMarketNativeComponent
 * @author RYG.Labs
 * @notice Component contract for NPC Market system using native token (ETH)
 * @dev Manages market data, inventory, and transaction records for ETH-based trading
 *
 * Key Features:
 * - Create and manage NPC markets with ETH payments
 * - Add/update/remove items from markets
 * - Track user purchase limits per item
 * - Manage item prices in wei
 * - Support both buying from and selling to NPCs
 * - Track transaction history and statistics
 */
contract NPCMarketNativeComponent {
    ...
    function recordTransaction(
        ...
        transactions[transactionId] = NPCMarketNativeStructs.TransactionRecord({
    ...
    function getMarketItem(
        ...
        NPCMarketNativeStructs.MarketItem storage item = npcMarkets[_npcId]
            .items[_itemId];
        return
            NPCMarketNativeStructs.MarketItemView({
                itemId: item.itemId,
    ...
    function getAllMarketItems(
        ...
        NPCMarketNativeStructs.MarketItemView[]
            memory items = new NPCMarketNativeStructs.MarketItemView[](
                itemIds.length
            );
        ...
        for (uint256 i = 0; i < itemIds.length; i++) {
            NPCMarketNativeStructs.MarketItem storage item = npcMarkets[_npcId]
                .items[itemIds[i]];
            items[i] = NPCMarketNativeStructs.MarketItemView({
                itemId: item.itemId,
                limitPerUser: item.limitPerUser,
                pricePerUnit: item.pricePerUnit,
                isSelling: item.isSelling,
    ...
    {
        NPCMarketNativeStructs.NPCMarket storage market = npcMarkets[_npcId];
        return (
            market.npcId,
            market.name,
            market.isActive,
